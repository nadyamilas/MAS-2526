...
for i in range(assembly.attributes["num_parts"]):
    ...
    #add here safe
    safe_trajectory = part.attributes["safe_trajectory"]
    #add here place
    place_trajectory = part.attributes["place_trajectory"]
    exit_trajectory = part.attributes["exit_trajectory"]

    # make sure gripper is open
    rtde.set_digital_io(4, False, ip = IP_ADDRESS)

    # pick up stickg
    rtde.move_to_joints(approach_pick_config, 0.5, 0.5, nowait=False, ip = IP_ADDRESS)
    rtde.move_to_joints(pick_config, 0.5, 0.5, nowait=False, ip = IP_ADDRESS)

    # close gripper
    rtde.set_digital_io(4, True, ip = IP_ADDRESS)
    time.sleep(1)
    ...
    # add here mpve to safe
    rtde.send_trajectory(safe_trajectory, 0.5, 0.5, 0.01, ip = IP_ADDRESS)
    # add here move to place
    rtde.send_trajectory(place_trajectory, 0.5, 0.5, 0.01, ip = IP_ADDRESS)
    ...
    rtde.send_trajectory(exit_trajectory, 0.5, 0.5, 0.01, ip = IP_ADDRESS)
